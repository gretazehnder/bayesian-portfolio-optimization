...
# Plug-in vs Bayesian (Normal-Inverse-Wishart) vs Equal Weight
...
stocks <- stocks %>%
  select(Date, where(~ sum(is.na(.)) <= 5))

cat("Assets rimasti:", ncol(stocks) - 1, "\n")
...
# ------------------------------------------------------------------
# 4.3  Equal Weight (1/N benchmark)
# ------------------------------------------------------------------
ew_weights <- function(R_window) {
  p <- ncol(R_window)
  rep(1 / p, p)
}

# ------------------------------------------------------------------
# 4.4  Performance metrics (MONTHLY data)
# ------------------------------------------------------------------
...
regime_perf <- regime_df %>%
  ...
  summarise(
    ...
    EW_Sharpe     = mean(EqualWeight) / sd(EqualWeight) * sqrt(12),
    Plugin_Return = mean(Plugin)      * 12,
    Bayes_Return  = mean(Bayesian)    * 12,
    EW_Return     = mean(EqualWeight) * 12
  )
...
roll_df <- data.frame(
  ...
  EqualWeight = sharpe_ew
) %>% drop_na()

# Bande di regime — solo periodi di crisi
...
p_rolling_sharpe <- ggplot(roll_df, aes(x = Date)) +
  ...
  geom_line(aes(y = EqualWeight, color = "Equal Weight"), linewidth = 0.7,
...
dist_df <- data.frame(
  ...
  Strategy = rep(c("Plugin", "Bayesian", "EqualWeight"), each = length(plugin_ret))
)
...
cat(strrep("-", 60), "\n")
cat(sprintf("%-25s %9.2f%% %9.2f%% %11.2f%%\n", "Annualised Return",
...
cat(sprintf("%-25s %10.4f %10.4f %12.4f\n", "Sharpe Ratio",
